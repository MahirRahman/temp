 ...
 **/
    ...
    public static function setWeather($location, $mapJson, $weatherJson) {
	    try {
            ...
            EXEC_SQL("insert into weather (DateTime, Location, MapJson, WeatherJson) values (?,?,?,?)",$today,$location, $mapJson, $weatherJson);
            $retData["status"]=0;
    ...
    public static function getWeather($date) {
        ...
        try {
            ...
            $retData["status"]=0;
            $retData["message"]="weather data for '$date' shown";
        }
        catch  (Exception $e) {
            $retData["status"]=1;
            $retData["message"]=$e->getMessage();
